...
<body>
    <div class="container mt-5">
        ...
        <form id="editProductForm" class="border p-4 rounded shadow">
            ...
            <button type="submit" class="btn btn-primary w-100 mb-3">Actualizar Producto</button>
    ...
    <script>
        ...
        function loadCategories(selectedCategoryId) {
            fetch('http://127.0.0.1:8000/categorias')
                ...
                .then(data => {
                    ...
                    categorySelect.innerHTML = '<option value="">Seleccione una categoría</option>'; // Limpiar opciones previas
                    data.forEach(category => {
                        const option = document.createElement('option');
                        option.value = category.id_cat;
                        option.textContent = category.nom_cat;
                        if (category.id_cat == selectedCategoryId) {
        ...
        fetch(`http://127.0.0.1:8000/productos/${productId}`)
            ...
            .then(product => {
                ...
                document.getElementById('prec').value = product.prec;
                document.getElementById('img').value = product.img;
                loadCategories(product.id_cat); // Cargar categorías y seleccionar la actual
            })
